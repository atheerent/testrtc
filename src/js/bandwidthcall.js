...
function BandwidthCall(config, test) {
  this.test = test;
  this.traceEvent = report.traceEventAsync('bandwidthcall');
  ...
  this.iceCandidateFilter_ = BandwidthCall.noFilter;
}

BandwidthCall.prototype = {
  ...
  onIceCandidate_: function(otherPeer, event) {
    if (event.candidate) {
      var parsed = BandwidthCall.parseCandidate(event.candidate.candidate);
...
BandwidthCall.noFilter = function() {
  return true;
};

BandwidthCall.isRelay = function(candidate) {
  return candidate.type === 'relay';
};

BandwidthCall.isNotHostCandidate = function(candidate) {
  return candidate.type !== 'host';
};

BandwidthCall.isReflexive = function(candidate) {
  return candidate.type === 'srflx';
};

BandwidthCall.isHost = function(candidate) {
  return candidate.type === 'host';
};

BandwidthCall.isIpv6 = function(candidate) {
  return candidate.address.indexOf(':') !== -1;
};

// Parse a 'candidate:' line into a JSON object.
BandwidthCall.parseCandidate = function(text) {
...
BandwidthCall.cachedIceServers_ = null;
// Keep track of when the request was made.
BandwidthCall.cachedIceConfigFetchTime_ = null;

// Get a TURN config, either from settings or from network traversal server.
BandwidthCall.asyncCreateTurnConfig = function(server, onSuccess, onError) {
  ...
  if (typeof(settings.turnURI) === 'string' && settings.turnURI !== '') {
    ...
    BandwidthCall.fetchTurnConfig_(server, function(response) {
...
BandwidthCall.asyncCreateStunConfig = function(onSuccess, onError) {
  var settings = currentTest.settings;
  if (typeof(settings.stunURI) === 'string' && settings.stunURI !== '') {
    ...
    BandwidthCall.fetchTurnConfig_(function(response) {
...
BandwidthCall.fetchTurnConfig_ = function(server, onSuccess, onError) {
  ...
  if (BandwidthCall.cachedIceServers_) {
    var isCachedIceConfigExpired =
      ((Date.now() - BandwidthCall.cachedIceConfigFetchTime_) / 1000 >
      parseInt(BandwidthCall.cachedIceServers_.lifetimeDuration)
      - testRunTime);

    if (server !== BandwidthCall.cachedIceServers_.iceServers[0].urls[0]) {
      isCachedIceConfigExpired = true;
    }

    if (!isCachedIceConfigExpired) {
      report.traceEventInstant('fetch-ice-config',
          'Using cached credentials.');
      onSuccess(BandwidthCall.getCachedIceCredentials_(server));
      return;
    }
  }

  BandwidthCall.cachedIceServers_ = {
  ...
  BandwidthCall.getCachedIceCredentials_ = function(server) {
    // Make a new object due to tests modifying the original response object.
    return JSON.parse(JSON.stringify(BandwidthCall.cachedIceServers_));
  };

  BandwidthCall.cachedIceConfigFetchTime_ = Date.now();
